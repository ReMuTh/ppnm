...
public class interpol {
	...
	static string endpoints = "akima";
	static double z;
	static bool zset = false;
	static int N = 101;
	static int n;
	static bool verbose = false;
	static akimaspline aspline;
	public static int Main(string[] args) {

		foreach(var arg in args){
			// parsing input parameters as in io-homework 
			var w=arg.Split(':');
			// endpoints options chooses between various schemes for the spine slope at beginning two points and end two points
			// defaults to akima
			if(w[0]=="-endpoints")endpoints=w[1];
			// -input sets the input file to read data from that needs interpolating
			else if(w[0]=="-input")infile=w[1];
